"""
8-Tile (8-Puzzle) Problem
Search Technique: Greedy Best First Search (GBFS)
Heuristic Used   : Number of Misplaced Tiles
"""
...
def misplaced_tiles(state, goal=GOAL_STATE):
    """Heuristic h(n): number of tiles not in their goal position (blank excluded)."""
    return sum(1 for i in range(9) if state[i] != 0 and state[i] != goal[i])


def get_neighbors(state):
    """Generate all states reachable by sliding a tile into the blank (0)."""
    neighbors = []
